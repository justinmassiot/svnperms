...
class Permission:
    ...
    def parse_groups(self, groupsiter):
        for option, value in groupsiter:
            groupusers = []
            for token in shlex.split(value): # split by spaces
                ...
                if token[0] == "@":
                    ...
                    if token[-1:] == ',': # support for "comma + space" separation style, for example the one generated by ldap-to-svn-authz
                        groupusers.append(token[:-1])
                    else:
